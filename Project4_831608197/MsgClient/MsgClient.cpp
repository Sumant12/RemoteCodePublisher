...
//                                                                     //
// Saisumanth Gopisetty, CSE687 - Object Oriented Design, Spring 2016  //
...
HttpMessage ClientHandler::readMessageForGet(HttpMessage& msg, Socket& socket) {
	...
	else if (getMessageType.compare("getFileNamesReply") == 0)
	{
		size_t numBytes = 0;
		size_t pos = msg.findAttribute("content-length");
		if (pos < msg.attributes().size())
		...
		acceptFileFromServer(msg, socket);
	}
...
bool ClientHandler::acceptFileFromServer(HttpMessage &msg, Socket& socket) {
	try {
...
HttpMessage ClientHandler::readMessage(Socket& socket)
{
	...
	try {
		...
		if (msg.attributes()[0].first == "POST")
			...
			msg = readMessageForGet(msg, socket);
...
void MsgClient::startClient(BQueue &sharedQueue)
{
	Utils::Title("Requirement5 Met: Client Program is started. Select tge files from the browse button and uplaod them to the Repository.");
	Utils::Title("Requirement5 Purposes Met: The files are uploaded to the Repository based on the project name as category.");
	Utils::Title("Requirement5 Purposes Met: Click on View categories to view the categories in the Repository");
	Utils::Title("Requirement5 Purposes Met: On clicking the download button files are fownloaded into clientRepository folder");
	Utils::Title("Requirement5 Purposes Met: On clicking the download button files are fownloaded into clientRepository folder");
	std::thread t2([&]() {
		try {
			SocketSystem ss;
...
void MsgClient::downloadScriptsOnStartUpOfClient()
{
...
void MsgClient::requestCodeAnaysis() {
	ClientCounter counter;
	size_t myCount = counter.count();
	std::string myCountString = Utilities::Converter<size_t>::toString(myCount);
...
void MsgClient::processGetAllFilesFromRepoForAParticluarCategory(string msg)
{
...
void MsgClient::processGetCategoriesMessageFromClient()
{
	try {
		cout << endl;
...
void MsgClient::processUploadMessageFromClient(string filepath)
{
	ClientCounter counter;
	size_t myCount = counter.count();
	std::string myCountString = Utilities::Converter<size_t>::toString(myCount);
