...
// ver 1.4                                                         //
//-----------------------------------------------------------------//
// Saisumanth Gopisetty(c) copyright 2016                          //
...
int AnalyzeCode(int argc, char* argv[]) {
	...
	//	codeExecutive.stopLogger();
		...
		/*pathForIndex += "/dependencyTable.htm";*/
		...
		/*pathForIndexInIIS += "/dependencyTable.htm";*/
...
bool sendFileToClient(string path,string FileName, int argc, char* argv[]) {
	try {
		HttpMessage sendMessageFromRepo;
		std::string ServerEndPoint = "localhost::8080";
		std::string ClientEndPoint = "localhost::8081";
		sendMessageFromRepo.addAttribute(HttpMessage::attribute("GET", "Message"));
		sendMessageFromRepo.addAttribute(HttpMessage::Attribute("mode", "oneWay"));
		sendMessageFromRepo.addAttribute(HttpMessage::parseAttribute("toAddr:" + ClientEndPoint));
		sendMessageFromRepo.addAttribute(HttpMessage::parseAttribute("fromAddr:" + ServerEndPoint));
		...
		{	cout<<"\n Server waiting to connect";
			::Sleep(100);}
		sendMessageFromRepo.addAttribute(HttpMessage::Attribute("messageType", "HtmFileContentReplyFromServer"));
		sendMessageFromRepo.addAttribute(HttpMessage::Attribute("fileName", FileName));
		sendMessageFromRepo.addAttribute(HttpMessage::Attribute("content-length", sizeString));
		std::string sendString = sendMessageFromRepo.toString();
		si.send(sendString.size(), (Socket::byte*)sendString.c_str());
...
bool LazyLoadingExec(string FileName, int argc, char* argv[]) {
	Utils::Title("Requirement 10 Met: Lazy loading is active. On downloading a file,its dependencies are also downloaded");
	try {
		if (CodeAnalysisExecutive::dependencyMapForUse.size() <= 1) {
			AnalyzeCode(argc, argv);
		}
		...
		for (it; it != CodeAnalysisExecutive::dependencyMapForUse.end(); it++) {
			...
			if (fileName_.compare(FileName) == 0) {
				...
				for (std::string str : it->second)
				{
					...
					sendFileToClient(FileHtmPath, dependendencyFile + ".htm", argc, argv);
...
void getMessagef_GetCategories(HttpMessage &msg, int argc, char* argv[]) {
...
void getMessage_getFileNames(HttpMessage &msg, int argc, char* argv[]) {
	...
	sendMessage.addAttribute(HttpMessage::Attribute("messageType", "getFileNamesReply"));
	if (sendResult.size() > 0)
	{
		sendMessage.addAttribute(HttpMessage::attribute("content-length", Converter<size_t>::toString(sendResult.size())));
		sendMessage.addBody(sendResult); 
		...
		if (contentSize > 0) {
			...
			if (getMsgBody.find('#') != std::string::npos) {
				...
				if (Split.find('#') != std::string::npos) {
					...
					LazyLoadingExec(FileName, argc, argv);
				}
			}
		}
	}
...
bool HandleGetMessages(HttpMessage &msg, int argc, char* argv[])
{
	string sendResult = "";
	HttpMessage sendMessage;
	std::string ServerEndPoint = "localhost::8080";
	...
	if (msg.findValue("messageType") != "")
	{
		...
		{
			getMessagef_GetCategories(msg, argc, argv);
		}
		else if (msg.findValue("messageType").compare("getFileNamesFromServer") == 0)
		{
			...
			sendFileToClient(FileHtmPath, jsfile, argc, argv);
			sendFileToClient(FileHtmPath, cssfile, argc, argv);
		}
	}
	return true;
}
...
void deleteFileFromServer(HttpMessage &msg, int argc, char* argv[]) {
			...
			if (getMsgBody.find('#') != std::string::npos) {
				...
				if (FirstSplit.find('#') != std::string::npos) {
					...
					if (FileSystem::File::exists(fullpath)) {
						...
						cout << "The requested path doesn't exist" << endl;
					}
...
bool processPostMessages(HttpMessage &msg, int argc, char* argv[]) {
	...
	if (msg.findValue("messageType") != "")
	{
		...
		else if (msg.findValue("messageType").compare("deleteFilesMessage") == 0) {
			cout << "\nServer needs to process the deleteFileFromServer message request from the client: \n";
			deleteFileFromServer(msg, argc, argv);
		}
	}
	return true;
}
...
*/
//This is Professor's code which is directly used to read file which is to be sent
	...
	try
	{
		...
		while (true)
		{
			...
			if (msg.attributes().size() > 0)
			{
				...
				{
					HandleGetMessages(msg, argc, argv);
				}
...
{	
	...
	Utils::Title("Requirement9 Met:: Repository and Client console wil act like test suites to demo all the requirements");
	Utils::Title("Requirement1 Met::The GUI is developd using WPF and all the rest of the projects are Windows console projects");
	Utils::Title("Requirement2 Met: using the C++ standard library's streams for all console I/O and new and delete for all heap-based memory management.");
	Utils::Title("Requirement3 Met:A Repository program that provides functionality to publish, as linked web pages, the contents of a set of C++ source code files. Please click on Analyze button to anaylyze the code and generate html files");
	Utils::Title("Requirement4 Met:TypeAnalysis,Dependency Analysis and publishing of pages is done as in Project3");
	Utils::Title("Requirement6 Met: All the communication happens throgh sockets. Please check the code ");
	Utils::Title("Requirement7 Met: All the messages being passed from Repository and client(Vice versa) are http style messages ");
	Utils::Title("Requirement8 Met: File upload and download is happening in bytes. Streams are estlabished with initial exchange of messsages. ");
	Utils::Title("Requirement10 Met: IIS is setup. Please read the readme file how to give inputs to run html files on IIS server");
