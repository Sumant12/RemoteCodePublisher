	...
	thread_ = std::thread(
		...
		while (!stop_)
		{
			...
			if (msg.find("FileUploadToRepo") != std::string::npos) {
				...
				msgClient.processUploadMessageFromClient(mshbeingsenttoclient);
			}
			else if (msg.find("getcategories") != std::string::npos)
	
				
				msgClient.processGetCategoriesMessageFromClient();
			 else if (msg.find("fetchFileNames") != std::string::npos) 
				
				msgClient.processGetAllFilesFromRepoForAParticluarCategory(msg);
	...
	std::thread theradInitClient([&]() {
		try {
			...
			{	BQueue& sharedQueue = pSendr->queue();
				MsgClient msgClient;
				msgClient.startClient(sharedQueue);
				msgClient.downloadScriptsOnStartUpOfClient();
			}
		}
